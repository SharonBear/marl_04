...
def q_learning_ucb_experiment(N=8, H=20, M=1001, gamma=0.99, samples=10, epsilon=0.1, tau=1.0, runs=10, restart_interval=1001):
    ...
    np.save("./npy/r_qlearning/r_qlearning_accuracies.npy", np.array(all_accuracies))
    np.save("./npy/r_qlearning/r_qlearning_rewards.npy", np.array(all_total_rewards))
    np.save("./npy/r_qlearning/r_qlearning_densities.npy", policy_to_facility_density(all_final_policies[-1], act_dic, state_dic))
    np.save("./npy/r_qlearning/r_qlearning_plot_matrix.npy", np.array(list(itertools.zip_longest(*all_accuracies, fillvalue=np.nan))).T)
    np.save("./npy/r_qlearning/r_qlearning_episode_rewards.npy", np.array(all_episode_rewards))
    np.save("./npy/r_qlearning/r_qlearning_agent_episode_rewards.npy", np.array(all_agent_rewards))
    ...
    plt.grid(True)
    plt.savefig("./pic/r_qlearning/r_qlearning_individual_runs.png", dpi=300)
    plt.close()

    pmean = np.nanmean(plot_accuracies, axis=0).tolist()
    pstdv = np.nanstd(plot_accuracies, axis=0).tolist()
    np.save("./npy/r_qlearning/r_qlearning_avg_mean.npy", np.array(pmean))
    np.save("./npy/r_qlearning/r_qlearning_avg_std.npy", np.array(pstdv))
    fig2 = plt.figure()
    ...
    plt.savefig("./pic/r_qlearning/r_qlearning_avg_runs.png", dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("./pic/r_qlearning/r_qlearning_facilities.png", dpi=300)
    plt.close()
    ...
    plt.grid(True)
    plt.savefig("./pic/r_qlearning/r_qlearning_cumulative_reward.png", dpi=300)
    plt.close()

    optimal_per_episode = N * 6
    regret_matrix = np.cumsum(optimal_per_episode - np.array(all_episode_rewards), axis=1)
    np.save("./npy/r_qlearning/r_qlearning_episode_regret.npy", regret_matrix)
    ...
    plt.grid(True)
    plt.savefig("./pic/r_qlearning/r_qlearning_episode_regret.png", dpi=300)
    plt.close()
    ...
    plt.grid(True)
    plt.savefig("./pic/r_qlearning/r_qlearning_mean_std_episode_reward.png", dpi=300)
    plt.close()
    ...
    plt.grid(True)
    plt.savefig("./pic/r_qlearning/r_qlearning_avg_regret_per_episode.png", dpi=300)
    plt.close()
    ...
    plt.grid(True)
    plt.savefig("./pic/r_qlearning/r_qlearning_avg_reward_per_episode.png", dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("./pic/r_qlearning/r_qlearning_per_agent_reward_curve.png", dpi=300)
    plt.close()
    # (前略，其餘部分不變)
    ...
    # ====== ✅ 儲存 npy 檔案 ======
    np.save("./npy/r_qlearning/r_qlearning_agent_reward_per_iteration.npy", all_agent_rewards)
    np.save("./npy/r_qlearning/r_qlearning_agent_cumulative_reward.npy", agent_cumulative)
    np.save("./npy/r_qlearning/r_qlearning_total_reward_per_iteration.npy", total_per_episode)
    np.save("./npy/r_qlearning/r_qlearning_total_cumulative_reward.npy", total_cumulative)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("./pic/r_qlearning/r_qlearning_per_agent_cumulative_reward.png", dpi=300)
    plt.close()
    ...
    plt.grid(True)
    plt.savefig("./pic/r_qlearning/r_qlearning_total_reward_per_episode.png", dpi=300)
    plt.close()
    ...
    plt.grid(True)
    plt.savefig("./pic/r_qlearning/s_r_qlearning_total_cumulative_reward.png", dpi=300)
    plt.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--i', type=int, default="75",
                        help="Choose restart interval: default=75")
    args = parser.parse_args()

    # print(f"Episode: {args.m}")
    # print(f"Restart interval: {args.i}")

    log_lines = []
    log_lines.append(f"Episode: {args.m}")
    log_lines.append(f"Restart interval: {args.i}")
    currentDateAndTime = datetime.now()
    formatted_time = currentDateAndTime.strftime("%Y-%m-%d %H:%M:%S")
    log_lines.append(f"<Restart_Qlearning> {formatted_time}")
    q_learning_ucb_experiment(N=8, H=80, M=args.m, epsilon=0.1, runs=10, restart_interval=args.i)
